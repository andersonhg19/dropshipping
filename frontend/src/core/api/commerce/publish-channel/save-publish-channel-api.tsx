...
import { BASE_URL_COMMERCE_PUBLISH_CHANNEL } from '@api/api-path'
import { RawFetchData, RawHttpError } from '@api/raw-fetch-data'
import { GenericSaveResponse } from '@api/generic-response'

async function SavePublishChannelApi(ae_object: Record<string, any>): Promise<GenericSaveResponse> {
  ...
  try {
    const options = {
      ...
      body: JSON.stringify({
        ...
        name: ae_object.name || '',
        type: ae_object.type || '',
        status: ae_object.status || 'INACTIVE',
        apiUrl: ae_object.apiUrl || '',
        apiKey: ae_object.apiKey || '',
        active: ae_object.active,
        idModifiedBy: ae_object.idModifiedBy || '',
      }),
    }

    const data = await RawFetchData<GenericSaveResponse>(BASE_URL_COMMERCE_PUBLISH_CHANNEL, 'save', options)
