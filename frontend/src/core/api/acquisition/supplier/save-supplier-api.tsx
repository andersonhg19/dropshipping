...
import { BASE_URL_ACQUISITION_SUPPLIER } from '@api/api-path'
import { RawFetchData, RawHttpError } from '@api/raw-fetch-data'
import { GenericSaveResponse } from '@api/generic-response'

async function SaveSupplierApi(ae_object: Record<string, any>): Promise<GenericSaveResponse> {
  ...
  try {
    const options = {
      ...
      body: JSON.stringify({
        ...
        name: ae_object.name || '',
        type: ae_object.type || '',
        country: ae_object.country || '',
        contactInfo: ae_object.contactInfo || '',
        shippingDaysMin: ae_object.shippingDaysMin ?? 0,
        shippingDaysMax: ae_object.shippingDaysMax ?? 0,
        active: ae_object.active,
        idModifiedBy: ae_object.idModifiedBy || '',
      }),
    }

    const data = await RawFetchData<GenericSaveResponse>(BASE_URL_ACQUISITION_SUPPLIER, 'save', options)
