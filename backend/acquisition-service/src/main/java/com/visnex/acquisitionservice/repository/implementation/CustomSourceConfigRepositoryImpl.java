...
import com.visnex.acquisitionservice.repository.CustomSourceConfigRepository;
...
public class CustomSourceConfigRepositoryImpl implements CustomSourceConfigRepository {
    ...
    @Override
    public Page<ResultSourceConfigDTO> findAllWithCriteria(SourceConfigFilterDTO filter, Pageable pageable) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();

        CriteriaQuery<SourceConfig> query = cb.createQuery(SourceConfig.class);
        Root<SourceConfig> root = query.from(SourceConfig.class);
        ...
        TypedQuery<SourceConfig> typedQuery = entityManager.createQuery(query);
        typedQuery.setFirstResult((int) pageable.getOffset());
        typedQuery.setMaxResults(pageable.getPageSize());

        List<SourceConfig> results = typedQuery.getResultList();

        CriteriaQuery<Long> countQuery = cb.createQuery(Long.class);
        Root<SourceConfig> countRoot = countQuery.from(SourceConfig.class);
        countQuery.select(cb.count(countRoot))
                .where(cb.and(buildPredicates(cb, countRoot, filter).toArray(new Predicate[0])));

        long total = entityManager.createQuery(countQuery).getSingleResult();

        List<ResultSourceConfigDTO> dtos = results.stream()
                .map(mapper::toDTO)
    ...
    private List<Predicate> buildPredicates(CriteriaBuilder cb, Root<SourceConfig> root, SourceConfigFilterDTO filter) {
