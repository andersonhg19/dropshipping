...
import com.visnex.commerceservice.mapper.PromptTemplateMapper;
...
public class PromptTemplateServiceImpl implements PromptTemplateService {
    ...
    @Override
    public ResultDTO saveAndUpdate(PromptTemplateDTO dto, String language) {
        ...
        try {
            ...
            if (dto.getId() == null) {
                Optional<PromptTemplate> existingByName = repository.findFirstByNameAndCompanyIdAndActive(dto.getName(), dto.getIdCompany(), true);
                if (existingByName.isPresent()) return new ResultDTO(false, m(Message.Msj.thisNameExists.toString(), lng), 101);
                PromptTemplate entity = mapper.toEntity(dto);
                if (entity.getActive() == null) entity.setActive(true);
                PromptTemplate created = repository.save(entity);
                ResultPromptTemplateDTO result = mapper.toDTO(created);
                completionUtils.enrich(result, lng);
                return new ResultDTO(result);
            }

            validation.requirePromptTemplate(dto.getId(), lng);
            Optional<PromptTemplate> conflict = repository.findFirstByNameAndCompanyIdAndActive(dto.getName(), dto.getIdCompany(), true);
            ...
            PromptTemplate updated = repository.save(mapper.toEntity(dto));
            ResultPromptTemplateDTO result = mapper.toDTO(updated);
            completionUtils.enrich(result, lng);
            return new ResultDTO(result);
    ...
    public ResultDTO getById(Long id, String language) {
        final String lng = lang(language);
        try {
            PromptTemplate entity = validation.requirePromptTemplate(id, lng);
            ResultPromptTemplateDTO result = mapper.toDTO(entity);
            completionUtils.enrich(result, lng);
            return new ResultDTO(result);
        } catch (ValidationException vex) { return new ResultDTO(false, vex.getMessage(), 102, vex.getClass().getSimpleName()); }
        catch (Exception e) { return new ResultDTO(false, m(Message.Msj.return_process_error.toString(), lng), 103, e.getMessage()); }
    ...
    @Override
    public ResultDTO getAllItems(PromptTemplateFilterDTO filterDTO, String language) {
        final String lng = lang(language);
        try {
            ...
            Page<PromptTemplate> pageResult = repository.findAllWithCriteria(filterDTO, PageRequest.of(page, size));
            if (pageResult == null || pageResult.isEmpty()) return new ResultDTO(new PageDTO<>(page, size, 0, Collections.emptyList()));

            List<ResultPromptTemplateDTO> dtos = pageResult.getContent().stream().map(mapper::toDTO).collect(Collectors.toList());
