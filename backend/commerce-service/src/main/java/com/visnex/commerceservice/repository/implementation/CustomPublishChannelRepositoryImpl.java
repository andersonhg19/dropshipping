...
import com.visnex.commerceservice.repository.CustomPublishChannelRepository;
...
public class CustomPublishChannelRepositoryImpl implements CustomPublishChannelRepository {

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Page<PublishChannel> findAllWithCriteria(PublishChannelFilterDTO filter, Pageable pageable) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();

        CriteriaQuery<PublishChannel> query = cb.createQuery(PublishChannel.class);
        Root<PublishChannel> root = query.from(PublishChannel.class);
        ...
        TypedQuery<PublishChannel> typedQuery = entityManager.createQuery(query);
        typedQuery.setFirstResult((int) pageable.getOffset());
        typedQuery.setMaxResults(pageable.getPageSize());

        List<PublishChannel> results = typedQuery.getResultList();

        CriteriaQuery<Long> countQuery = cb.createQuery(Long.class);
        Root<PublishChannel> countRoot = countQuery.from(PublishChannel.class);
    ...
    private List<Predicate> buildPredicates(CriteriaBuilder cb, Root<PublishChannel> root, PublishChannelFilterDTO filter) {
        ...
        PredicateBuilderUtil.addEquals(predicates, cb, root, "autoSync", filter.getAutoSync());
