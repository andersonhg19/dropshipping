...
import com.visnex.commerceservice.mapper.CategoryMapper;
import com.visnex.commerceservice.repository.CategoryRepository;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    @Override
    public ResultDTO saveAndUpdate(CategoryDTO dto, String language) {
        ...
        try {
            ...
            if (dto.getId() == null) {
                Optional<Category> existingByName = repository.findFirstByNameAndCompanyIdAndActive(dto.getName(), dto.getIdCompany(), true);
                ...
                Category created = repository.save(entity);
                ResultCategoryDTO result = mapper.toDTO(created);
                completionUtils.enrich(result, lng);
                return new ResultDTO(result);
            }

            validation.requireCategory(dto.getId(), lng);
            Optional<Category> conflict = repository.findFirstByNameAndCompanyIdAndActive(dto.getName(), dto.getIdCompany(), true);
            ...
            Category updated = repository.save(mapper.toEntity(dto));
            ResultCategoryDTO result = mapper.toDTO(updated);
            completionUtils.enrich(result, lng);
            return new ResultDTO(result);
    ...
    public ResultDTO getById(Long id, String language) {
        final String lng = lang(language);
        try {
            Category entity = validation.requireCategory(id, lng);
            ResultCategoryDTO result = mapper.toDTO(entity);
            completionUtils.enrich(result, lng);
            return new ResultDTO(result);
        } catch (ValidationException vex) { return new ResultDTO(false, vex.getMessage(), 102, vex.getClass().getSimpleName()); }
        catch (Exception e) { return new ResultDTO(false, m(Message.Msj.return_process_error.toString(), lng), 103, e.getMessage()); }
    ...
    @Override
    public ResultDTO getAllItems(CategoryFilterDTO filterDTO, String language) {
        final String lng = lang(language);
        try {
            ...
            Page<Category> pageResult = repository.findAllWithCriteria(filterDTO, PageRequest.of(page, size));
            if (pageResult == null || pageResult.isEmpty()) return new ResultDTO(new PageDTO<>(page, size, 0, Collections.emptyList()));

            List<ResultCategoryDTO> dtos = pageResult.getContent().stream().map(mapper::toDTO).collect(Collectors.toList());
